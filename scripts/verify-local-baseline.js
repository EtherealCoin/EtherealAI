...
function checkExchangeProductionExecutionModule() {
  ...
  const phase6CVerificationFixture = {
    exchangeName: 'binance',
    displayName: 'Binance',
    status: 'Trading Permission Present But Locked',
    connectionStatus: 'production_account_verified',
    ...
    proof: {
      symbolRules: { loaded: true, minOrderSize: 0.00001, minNotionalUsd: 5 },
      fees: { loaded: true, fallbackFeePercent: 0.1 },
      marketData: {
        loaded: true,
        bidPrice: 49995,
        askPrice: 50005,
    ...
    checksPassed: ['API connected', 'Withdrawals disabled'],
    checksFailed: []
  };
  const phase6CDryRunProof = buildPhase6CProductionDryRunProof({
    order,
    credentialVerification: phase6CVerificationFixture,
    safety,
  ...
  const phase6CEligibility = buildPhase6CTinyLiveEligibility({
    credentialVerification: phase6CVerificationFixture,
    dryRunProof: phase6CDryRunProof,
    riskProfile: { id: 1, status: 'active', kill_switch_enabled: 0 }
  });
  const phase6CWizard = buildPhase6CWizard({
    connectors: [{ id: 2, exchange_name: 'kraken', settings: { registryId: 'kraken', productionConnection: { referenceName: 'fixture', connectionStatus: 'production_account_verified' } } }],
  ...
  const phase6DKrakenReadiness = {
    ...
    criticalPassed: true,
    withdrawalPermissionDetected: false,
    marginOrLeverageDetected: false,
    futuresDetected: false,
    balancesReadable: true,
  ...
  const phase6EPassedDryRunProof = {
    passed: true,
    productionOrderEndpointCalled: false,
    status: 'Dry-run proof passed'
  };
  const phase6EPassedPreflight = {
    technicalReady: true,
    readyToPlace: false,
    productionOrderEndpointCalled: false,
    plainEnglishStatus: 'Dry-run passed and tiny live test is technically eligible; owner approval remains separate.'
  };
  const phase6EFinalStatus = buildPhase6EFinalStatus({
    ...
    connector: {
      ...
      settings: {
        ...
        productionConnection: {
          ...
          phase6EReadiness: phase6DKrakenReadiness,
          phase6EDryRunProof: phase6EPassedDryRunProof,
          phase6EPreflight: phase6EPassedPreflight,
          phase6ESimulationPreview: phase6DSimulationPreview
        }
      }
    },
    vaultStatus: { entries: [{ exchangeName: 'kraken' }] },
  ...
  const phase6FSimulationPreview = {
    ...phase6DSimulationPreview,
    expectedNotionalUsd: 5,
    expectedFeesUsd: 0.013,
    expectedSlippagePercent: 0.02,
    expectedQuantity: 0.0001,
    expectedFillPrice: 50005,
    feePercent: 0.26,
    abortConditions: [],
    expectedRemainingBalance: { asset: 'ZUSD', beforeUsd: 100, afterUsd: 94.987 }
  };
  const phase6FTinyLivePreview = buildPhase6FTinyLivePreview({
    simulationPreview: phase6FSimulationPreview,
    krakenReadiness: phase6DKrakenReadiness,
    policy: DEFAULT_PHASE6D_TINY_LIVE_POLICY
  });
  const phase6FOperatorResult = buildPhase6FOperatorResult({
    connector: {
      id: 3,
      exchange_name: 'kraken',
      settings: {
        ...
        productionConnection: {
          ...
          phase6FPreflight: phase6EPassedPreflight,
  ...
  if (
    ...
    || PHASE6E_RECOMMENDED_FIRST_EXCHANGE !== 'kraken'
    || PHASE6F_RECOMMENDED_FIRST_EXCHANGE !== 'kraken'
    || PHASE6F_ENABLE_CONFIRMATION_PHRASE !== 'I ENABLE KRAKEN TINY LIVE TEST MODE'
    ...
    || phase6CDryRunProof.title !== 'Production Dry-Run Proof'
    || phase6CDryRunProof.passed !== true
    || phase6CDryRunProof.productionOrderEndpointCalled !== false
    || phase6CEligibility.title !== 'Tiny Live Test Eligibility'
    || phase6CEligibility.eligibleExceptFinalConfirmation !== true
    ...
    || phase6FOperatorResult.safetyBoundary.withdrawalsEnabled !== false
    || !phase6FOperatorResult.operatorResults.some(item => item.label === 'Your key works safely' && item.passed === true)
...
function checkLiveTradingLaunchCenterUi() {
  ...
  if (
    ...
    || !html.includes('Phase 6C: Real Credential Verification And Dry-Run Proof')
    || !html.includes('Start Phase 6C Safely')
    || !html.includes('Step 2: Add API Key Safely')
    || !html.includes('Step 3: Verify Real Exchange Credentials')
    ...
    || !html.includes('Kraken Live Setup Walkthrough')
    || !html.includes('Start Kraken Live Setup Walkthrough')
    || !html.includes('Create restricted Kraken spot key')
    || !html.includes('Confirm withdrawals and transfers are off')
    || !html.includes('Confirm margin, futures, and leverage are off')
    ...
    || !html.includes("'phase6e-verify-key': verifyPhase6EKrakenKey")
    || !html.includes('Withdraw Funds is disabled')
    || !html.includes('Kraken Private Key')
    || !html.includes('No production dry-run proof has run yet. Endpoint called: No.')
    || !html.includes('Phase 6F: First Authenticated Kraken Connection And Tiny Live Eligibility')
    || !html.includes('Verify Kraken Connection')
    ...
    || !html.includes('Phrase matched')
    || !html.includes('PLACE ONE TINY LIVE KRAKEN ORDER')
    || !html.includes('Live Execution Audit State')
    || !html.includes('Live Execution Journal')
    || !html.includes('Post-Trade Verification')
    || !html.includes('ENDPOINT CALLED: NO')
    || !html.includes('one real Kraken spot order within the tiny owner limit')
    || !html.includes('Emergency Stop Tiny Test')
    || !html.includes('I APPROVE THIS KRAKEN TINY LIVE TEST')
    || !html.includes('window.EtherealAIKrakenTinyLiveTest')
    || !html.includes('KRAKEN_TINY_LIVE_CONFIRMATION_PHRASE')
    || !html.includes('KRAKEN_TINY_LIVE_MAX_USD')
    || !html.includes('isKrakenTinyLiveReadyPreview')
    || !html.includes('deriveKrakenTinyLiveReadyPreview')
    || !html.includes('deriveKrakenTinyLivePreviewId')
    || !html.includes('collectKrakenTinyFinalApprovalState')
    || !html.includes('previewGateReady')
    || !html.includes('data-final-approval-enabled')
    || !html.includes('data-kraken-tiny-disable-reason')
    || !html.includes('copyKrakenTinyConfirmationPhrase')
    || !html.includes('handleKrakenTinySimpleAction')
    || !html.includes('getFinalApprovalState: collectKrakenTinyFinalApprovalState')
    || !html.includes('updateFinalApprovalButton: updateKrakenTinyPlaceButton')
    || !html.includes("addEventListener('keyup', updateKrakenTinyPlaceButton)")
    ...
    || !html.includes("'kraken-tiny-preview': previewKrakenTinyLiveTest")
    ...
    || !html.includes('/api/v1/live-trading-launch/kraken-tiny-live-test/status')
    || !html.includes('/api/v1/live-trading-launch/kraken-tiny-live-test/preview')
    || !html.includes('/api/v1/live-trading-launch/kraken-tiny-live-test/place')
    || !html.includes('/api/v1/live-trading-launch/kraken-tiny-live-test/orders/')
    || !html.includes('/api/v1/live-trading-launch/kraken-tiny-live-test/emergency-stop')
    ...
    || !styles.includes('.phase6c-wizard')
    || !styles.includes('.phase6c-summary-grid')
    ...
    || !styles.includes('.kraken-live-setup-walkthrough')
    ...
    || !styles.includes('.phase6e-key-panel')
    || !styles.includes('.phase6e-proof-panel')
    || !styles.includes('.phase6e-check-item')
    || !styles.includes('.phase6f-wizard')
    ...
    || !styles.includes('.kraken-locked-button')
    || !styles.includes('.kraken-tiny-journal-list')
    || !styles.includes('.danger-action')
    ...
    || !routes.includes("app.get('/api/v1/live-trading-launch/kraken-tiny-live-test/status'")
    || !routes.includes("app.post('/api/v1/live-trading-launch/kraken-tiny-live-test/preview'")
    || !routes.includes("app.post('/api/v1/live-trading-launch/kraken-tiny-live-test/place'")
    || !routes.includes("app.get('/api/v1/live-trading-launch/kraken-tiny-live-test/orders/:id/status'")
    || !routes.includes("app.post('/api/v1/live-trading-launch/kraken-tiny-live-test/orders/:id/cancel'")
    || !routes.includes("app.post('/api/v1/live-trading-launch/kraken-tiny-live-test/emergency-stop'")
    || !routes.includes('LIVE EXECUTION OCCURRED')
    || !routes.includes('one_live_order_unused')
    || !routes.includes('buildKrakenTinyLiveAuditState')
    || !routes.includes('getKrakenTinyLiveExecutionJournal')
    ...
    || !server.includes('PHASE6E_RECOMMENDED_FIRST_EXCHANGE')
    || !server.includes('KRAKEN_PHASE6E_WALKTHROUGH')
    || !server.includes('buildPhase6EFinalStatus')
    || !server.includes('buildPhase6EWalkthrough')
    || !server.includes('PHASE6F_RECOMMENDED_FIRST_EXCHANGE')
    ...
    || !routeRegistration.includes('phase6ERecommendedFirstExchange')
    || !routeRegistration.includes('krakenPhase6EWalkthrough')
    || !routeRegistration.includes('buildPhase6EFinalStatus')
    || !routeRegistration.includes('buildPhase6EWalkthrough')
    || !routeRegistration.includes('phase6FRecommendedFirstExchange')
    ...
    || !phase6Production.includes('PHASE6C_RECOMMENDED_FIRST_EXCHANGE')
    || !phase6Production.includes('verifyProductionExchangeCredentials')
    || !phase6Production.includes('fetchKrakenPhase6CProof')
    ...
    || !phase6Production.includes('PHASE6E_RECOMMENDED_FIRST_EXCHANGE')
    || !phase6Production.includes('KRAKEN_PHASE6E_WALKTHROUGH')
    || !phase6Production.includes('buildPhase6EFinalStatus')
    || !phase6Production.includes('buildPhase6EWalkthrough')
...
function checkSimpleOperatorModeUsabilityRefactor() {
  ...
  if (
    ...
    || !operatorMode.includes('showLaunchPipeline: true')
    || !operatorMode.includes('operator-launch-pipeline-widget')
    || !operatorMode.includes('renderLaunchPipelineWidget')
    || !operatorMode.includes('formatLaunchBlocker')
    || !operatorMode.includes('Continue Launch Project')
    || !operatorMode.includes('Review Launch Package')
    || !operatorMode.includes('/api/v1/token-launch-pipeline/state')
    ...
    || !operatorMode.includes("THEME_KEY = 'etherealai.themeMode'")
    || !operatorMode.includes('function applyTheme')
    || !operatorMode.includes('topThemeDropdownMarkup')
    || !operatorMode.includes('data-operator-theme="auto"')
    || !operatorMode.includes('data-operator-theme="day"')
    || !operatorMode.includes('data-operator-theme="night"')
    || !operatorMode.includes('Auto Mode')
    || !operatorMode.includes('Light Mode')
    || !operatorMode.includes('Dark Mode')
    ...
    || !operatorMode.includes('data-operator-home-base')
    ...
    || !operatorMode.includes('href="/logout"')
    || !operatorMode.includes('renderIdentityBand')
    || !operatorMode.includes('Operational systems layer')
    || !operatorMode.includes('Safe: local paper and planning')
    || !operatorMode.includes('Warning: owner review needed')
    || !operatorMode.includes('Live: locked until approval')
    || !operatorMode.includes('operator-status-safe')
    || !operatorMode.includes('operator-status-warning')
    || !operatorMode.includes('operator-status-live')
    ...
    || !operatorMode.includes('/public/brand/ethereal-logo-generator-example.png')
    ...
    || !operatorMode.includes('Token Launch Factory')
    || !operatorMode.includes('Start Token Launch Pipeline')
    || !operatorMode.includes('Save Local Token Project')
    || !operatorMode.includes('website-whitepaper-roadmap-builder')
    || !operatorMode.includes('launch-package-review')
    ...
    || !styles.includes('True day/night operator theme architecture')
    || !styles.includes('--theme-background')
    || !styles.includes('--theme-panel-background')
    || !styles.includes('--theme-card-background')
    || !styles.includes('--theme-primary-text')
    || !styles.includes('--theme-secondary-text')
    || !styles.includes('--theme-muted-text')
    || !styles.includes('--theme-button-background')
    || !styles.includes('--theme-button-hover')
    || !styles.includes('--theme-button-text')
    || !styles.includes('--theme-danger')
    || !styles.includes('--theme-success')
    || !styles.includes('--theme-warning')
    || !styles.includes('.operator-theme-dropdown')
    || !styles.includes('.operator-launch-widget')
    || !styles.includes('.operator-launch-widget-grid')
    || !styles.includes('.operator-theme-active')
    ...
    || !styles.includes('.token-workbench-subnav')
    || !styles.includes('.token-two-gate-summary')
    || !styles.includes('.token-workbench-section-heading')
    || !styles.includes('.token-workbench-subsection')
    || !styles.includes('.token-builder-surface')
    ...
    || !dashboard.includes('Active Local Launch Project')
    || !dashboard.includes('Continue Launch Project')
    ...
    || !styles.includes('Closer Ethereal dapp shell pass')
    ...
    || !styles.includes('.operator-identity-band')
    || !styles.includes('.operator-state-safe')
    || !styles.includes('.operator-state-warning')
    || !styles.includes('.operator-state-live')
    || !styles.includes('ethereal-real-dark-logo.png')
    || !styles.includes('body.operator-simple-mode .operator-status-safe')
    || !styles.includes('body.operator-simple-mode .operator-status-live')
    || !styles.includes('body.operator-simple-mode .operator-workspace-actions')
    || !styles.includes('grid-template-columns: repeat(2, minmax(160px, 1fr))')
    || !styles.includes('.operator-training-menu-list[hidden]')
    || !styles.includes('.operator-black-hole-logout')
    || !styles.includes('.operator-black-hole-icon')
    || !styles.includes('.operator-top-dropdown')
    || !styles.includes('.operator-top-control')
    || !styles.includes('.operator-mode-toggle')
    || !styles.includes('.operator-active-page')
    || !styles.includes('.hero-action-grid')
    || !styles.includes('.hero-button-equal')
    || !styles.includes('.hero-button-primary-wide')
    || !styles.includes('body.operator-simple-mode > nav a[href="/logout"]')
    ...
    || !styles.includes('.api-operator-wizard')
    || !styles.includes('.api-owner-step-grid')
    ...
    || !styles.includes('.token-launch-wide-controls')
...
function checkLocalOnlySurfaceCues() {
  ...
  if (
    ...
    || !solidity.includes('Hybrid meme + utility token')
    || !solidity.includes('Dapp plan')
    || !solidity.includes('build three dapp modules now')
    || !solidity.includes('Total supply')
    || !solidity.includes('Initial minted supply')
    || !solidity.includes('Supply model')
    || !solidity.includes('Mintable with capped owner-approved release')
    || !solidity.includes('Admin model')
    || !solidity.includes('Burn function')
    || !solidity.includes('Pause function')
    || !solidity.includes('Transfer fee / tax')
    || !solidity.includes('Staking / rewards')
    || !solidity.includes('Treasury allocation')
    || !solidity.includes('Liquidity allocation')
    || !solidity.includes('Team allocation')
    || !solidity.includes('Community / airdrop allocation')
    || !solidity.includes('Vesting / release schedule')
    || !solidity.includes('Use-case detail')
    ...
    || !solidity.includes('token-workbench-subnav')
    || !solidity.includes('token-two-gate-summary')
    || !solidity.includes('token-launch-market-supply-notes')
    || !solidity.includes('token-launch-multi-chain-plan')
    || !solidity.includes('token-launch-launch-timeline')
    || !solidity.includes('token-launch-listing-ambition')
    || !solidity.includes('token-dapp-modules-panel')
    ...
    || !solidity.includes('launch-pipeline-progress')
    || !solidity.includes('Continue Next Step')
    || !solidity.includes('Review Launch Package')
    || !solidity.includes('loadLaunchPipelineState')
    || !solidity.includes('/api/v1/token-launch-pipeline/state')
    || !solidity.includes('/launch-package')
    || !solidity.includes('Launch Package Review')
    || !solidity.includes('Complete Local Token Launch Package')
    || !solidity.includes('renderLaunchPackageReview')
    || !solidity.includes('Export Local Package')
    ...
    || !solidity.includes('website-whitepaper-roadmap-builder')
    || !solidity.includes('token-website-builder-panel')
    || !solidity.includes('token-whitepaper-builder-panel')
    || !solidity.includes('token-roadmap-builder-panel')
    || !solidity.includes('Apply Token Data To Website')
    || !solidity.includes('Generate Premium Whitepaper Draft')
    || !solidity.includes('Generate Realistic Roadmap Draft')
    || !solidity.includes('Save Website / Whitepaper Draft')
    || !solidity.includes('Refresh Local Website Preview')
    || !solidity.includes('token-site-local-preview')
    || !solidity.includes('renderTokenSiteLocalPreview')
    || !solidity.includes('token-site-roadmap')
    || !solidity.includes('token-site-whitepaper')
    || !solidity.includes('launch-review-edit-whitepaper')
    || !solidity.includes('launch-review-edit-roadmap')
    || !solidity.includes('logo-studio-choice-grid')
    || !solidity.includes('Choose This Local Logo Direction')
    || !solidity.includes('pre-listing selected')
    || !solidity.includes('No external image generation')
    ...
    || !solidity.includes('token-logo-text-influence')
    || !solidity.includes('token-logo-image-influence')
    || !solidity.includes('token-logo-style-influence')
    || !solidity.includes('token-logo-readability-preview')
    || !solidity.includes('Regenerate 3 Local Logo Specs')
    || !solidity.includes('Continue To Website Builder')
    ...
    || !solidity.includes('token-site-faq')
    ...
    || !solidity.includes('token-site-tone')
    || !solidity.includes('Generate Website Intelligence Draft')
    || !solidity.includes('Regenerate Selected Whitepaper Section')
    || !solidity.includes('Roadmap Intelligence')
    || !solidity.includes('Save Roadmap Phases')
    || !solidity.includes('Generate Social Launch Kit Drafts')
    || !solidity.includes('token-site-inspiration-patterns')
    || !solidity.includes('No live CoinMarketCap Top 200 analysis has been performed')
    || !solidity.includes('Deploy Contract · Locked')
    || !solidity.includes('Publish Website · Locked')
    || !solidity.includes('renderMarketInspirationBrief')
    || !solidity.includes('buildPremiumWhitepaperSections')
    || !solidity.includes('buildRoadmapPhases')
    || !solidity.includes('buildWebsiteSections')
    || !solidity.includes('buildTopTokenPatternCards')
    || !solidity.includes('buildSocialLaunchKitDraft')
  ...
  if (
    ...
    || !tokenEcosystem.includes('websiteWhitepaperRoadmap')
    || !tokenEcosystem.includes('generatedWhitepaperSections')
    || !tokenEcosystem.includes('generatedWebsiteSections')
    || !tokenEcosystem.includes('generatedRoadmapPhases')
    || !tokenEcosystem.includes('inspirationPatterns')
    || !tokenEcosystem.includes('socialLaunchKit')
    || !tokenEcosystem.includes('liveTop200AnalysisPerformed: false')
    || !tokenEcosystem.includes('TOKEN_LAUNCH_PIPELINE_STEPS')
    || !tokenEcosystem.includes('buildTokenLaunchPipelineState')
    || !tokenEcosystem.includes('buildTokenLaunchPackageReview')
    || !tokenEcosystem.includes('buildWebsiteDraftCompletion')
    || !tokenEcosystem.includes('real-world-mark')
    || !tokenEcosystem.includes('orbital-ai-core')
    || !tokenEcosystem.includes('we-math-better')
    || !tokenEcosystem.includes('externalImageGenerationEnabled: false')
