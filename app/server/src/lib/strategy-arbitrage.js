...
function buildVenueQuote(candle, venue, index, rules, strategyType, role = 'neutral') {
...
function buildVenueExecution(quote, quantity, side) {
  ...
  if (side === 'sell') {
    const proceeds = notional - feeCost - slippageCost;
  ...
  const totalCost = notional + feeCost + slippageCost;
...
function evaluateArbitrageOpportunity(candle, index, strategy, rules, equity) {
  const buyQuotes = rules.buyVenues.map(venue => buildVenueQuote(candle, venue, index, rules, strategy.strategy_type, 'buy'));
  const sellQuotes = rules.sellVenues.map(venue => buildVenueQuote(candle, venue, index + 3, rules, strategy.strategy_type, 'sell'));
  const candidates = [];
  ...
  for (const buyQuote of buyQuotes) {
    for (const sellQuote of sellQuotes) {
      ...
      const buyRoute = buildVenueExecution(buyQuote, quantity, 'buy');
      const sellRoute = buildVenueExecution(sellQuote, quantity, 'sell');
  ...
  for (let index = 0; index < normalizedCandles.length; index += 1) {
    ...
    if (best?.passed) {
      ...
      decisionLog.push({
        ...
        reason: 'net_spread_liquidity_latency_passed',
