...
- `/security-lockdown` is the non-coder Mac Security Lockdown Center. It runs read-only local Mac hardening audits, separates visible human admin accounts from built-in/system admin members, shows priority owner actions, lists startup persistence and listening TCP services, gives hostile-network emergency containment steps, includes suspected admin/kernel compromise handling, and keeps all privileged remediation as visible owner action.
...
- Safe user-level Mac hardening applied in this session: immediate password after sleep/screensaver, AirDrop disable preference, visible filename extensions, and Finder extension-change warnings. Safari safe-download auto-open could not be changed by `defaults` due container preference write restrictions and remains a manual browser setting.
