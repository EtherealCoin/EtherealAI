...
- `/security-lockdown` is the non-coder Mac Security Lockdown Center. It runs read-only local Mac hardening audits, shows priority owner actions, lists listening TCP services, gives hostile-network emergency containment steps, and keeps all privileged remediation as visible owner action.
