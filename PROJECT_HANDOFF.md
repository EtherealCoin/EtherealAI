...
- Strategy Lab now has a structured arbitrage strategy lane for `Cross-Exchange Arbitrage`, `Cross-DEX Arbitrage`, and `Hybrid DEX/CEX Arbitrage`. The UI collects buy venues, sell venues, arbitrage type, minimum spread, estimated fees, slippage tolerance, minimum liquidity, max latency, simultaneous execution, stablecoin pair, and token whitelist. The backend stores these rules in `trading_strategies.strategy_type` and `strategy_rules_json`, bypasses the indicator parser for arbitrage, and runs local fee/slippage/liquidity/latency-aware route simulations with no live orders or wallet signing.
