...
- `/owner-setup` is the non-coder Setup Wizard / Owner Setup Wizard. It uses `/api/v1/owner-setup-wizard` to show paper trading progress from `95%` to `100%` and full E2E setup readiness from `72%` to `100%` while keeping live trading, wallet signing, seed phrases, private keys, and secret-value display disabled. It reads approved credential names/presence from `~/EtherealAI_Secrets/.env`, has a visual `.env` file picker that verifies selected file contents locally in the browser without sending values to the server, displays only detected public wallet addresses, creates a `.env.example` template, verifies each blocked gate with one click, can run one paper-only verification cycle, and saves public wallet metadata through `/api/v1/wallets`.
...
- `/strategy-lab#bot-automation` is now the non-coder Bot Operator Wizard. It walks the owner through strategy, passed paper session, active paper risk profile, safe paper connector, ready paper plan, active paper schedule, and final Owner Setup verification. It also includes a one-click `Use Safe Defaults and Finish Paper Setup` path that creates only local paper records and keeps live trading, wallet signing, exchange orders, token deployment, and social posting disabled.
...
- Simple Operator Mode is now the default UX layer on the main app pages. It shows a large page-specific operator workspace, one primary action, plain-English status tiles, simplified task cards, and a `Show me how` tutorial overlay. Advanced Developer Mode is explicitly available for raw logs, JSON, route tables, archived/debug records, IDs, internal gates, diagnostics, and developer filters.
...
- New client script `app/client/js/operator-next-action.js` powers the global next-action assistant. It reads only local protected APIs and prioritizes login, live-safety review, security fixes, paper setup completion, wallet public metadata, full setup gates, paper bot monitoring, and Mission Control.
- New client script `app/client/js/operator-mode.js` powers Simple Operator Mode, Advanced Developer Mode, page-specific guided workflows, hidden advanced clutter, and built-in tutorial overlays.
