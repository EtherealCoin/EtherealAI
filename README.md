...
- The Phase 6E UI shows exactly where to go in Kraken, which permissions to enable, which permissions must stay disabled, and plain-English explanations for each setting. It saves Kraken API key values only to the encrypted production vault, never redisplays the secret, supports delete/rotate, verifies the key, runs a no-order production dry-run proof, and reports one of: `Not ready`, `Verified but dry-run missing`, `Dry-run passed, tiny live test eligible`, or `Blocked because unsafe permission detected`.
...
- The Phase 6F UI shows `Verify Kraken Connection`, `Build Tiny Live Preview`, and `Enable Tiny Live Test Mode`. It translates Kraken auth/signature/IP/permission/balance/market metadata outcomes into plain English, builds a no-order tiny preview with estimated size, fees, slippage, minimum trade rules, exposure, and abort conditions, and keeps execution blocked even after typed owner confirmation.
