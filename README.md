...
- Phase 6B/6C now recommends Kraken first for the real owner credential path because it has a simpler restricted API key/private-key flow and a permission-info endpoint suitable for proving safety before any orders.
...
- `GET /api/v1/live-trading-launch/phase6c/wizard`, `POST /api/v1/live-trading-launch/phase6c/verify-credentials`, and `POST /api/v1/live-trading-launch/phase6c/dry-run-proof` add the real proof path: choose one exchange, save a key to the encrypted vault, verify permissions/balances/fees/symbol rules/rate notes, run a no-order production dry-run proof, and review tiny-live eligibility.
- Kraken Phase 6C verification reads `/0/private/Balance`, `/0/private/GetApiKeyInfo`, `/0/public/AssetPairs`, `/0/private/TradeVolume` where permission allows it, `/0/public/Ticker`, and `/0/public/Depth`. It rejects unsafe withdrawal/transfer/margin/futures/leverage signals and records proof snapshots without returning secret values to the browser.
- Phase 6C dry-run proof checks exchange, symbol, side, tiny amount, balance, minimum order size, fee estimate, live price, spread/slippage, risk profile, kill switch, withdrawal-disabled proof, and confirms the production endpoint remains blocked.
- Phase 6D adds the first real Kraken authenticated-readiness and tiny-live-test framework: `GET /api/v1/live-trading-launch/phase6d/framework`, `POST /phase6d/kraken-authenticated-readiness`, `POST /phase6d/prepare-tiny-live-test`, `POST /phase6d/validate-tiny-live-test`, `POST /phase6d/arm-tiny-live-test`, and `POST /phase6d/emergency-stop`.
- Kraken Phase 6D reads balances, API key info, pair rules, trade-volume fees when allowed, trade balance/open orders when allowed, system status, ticker, and depth. It blocks unsafe withdrawal, transfer, margin, futures, leverage, and master/full-access key signals.
- Phase 6D preflight builds an exact live-order simulation preview with expected fill price, fees, slippage, quantity, remaining balance, latency risk, and abort conditions. It writes audit/ledger evidence but keeps `production_order_endpoint_called = 0` and `production_order_endpoint_enabled = 0`.
- Phase 6E adds the non-coder Kraken API key walkthrough: `GET /api/v1/live-trading-launch/phase6e/walkthrough`, `POST /phase6e/save-kraken-key`, `DELETE /phase6e/kraken-key`, `POST /phase6e/verify-kraken-key`, and `POST /phase6e/dry-run-proof`.
- The Phase 6E UI shows exactly where to go in Kraken, which permissions to enable, which permissions must stay disabled, and plain-English explanations for each setting. It saves Kraken API key values only to the encrypted production vault, never redisplays the secret, supports delete/rotate, verifies the key, runs a no-order production dry-run proof, and reports one of: `Not ready`, `Verified but dry-run missing`, `SAFE READY FOR OPTIONAL TINY LIVE TEST`, or `Blocked because unsafe permission detected`.
- Phase 6E still cannot place a real order. It keeps Kraken AddOrder, unrestricted live trading, withdrawals/transfers, wallet signing, margin, futures, leverage, automation loops, autonomous retry, recurring trades, and scaling disabled until a later explicit owner-approved tiny live test phase.
- Phase 6F adds the first safe authenticated Kraken readiness layer: `GET /api/v1/live-trading-launch/phase6f/status`, `POST /phase6f/authenticated-readiness`, `POST /phase6f/tiny-live-preview`, and `POST /phase6f/enable-tiny-live-test-mode`.
- The Phase 6F UI shows `Verify Kraken Connection`, `Build Tiny Live Preview`, and `Enable Tiny Live Test Mode`. It translates Kraken auth/signature/IP/permission/balance/market metadata outcomes into plain English, builds a no-order tiny preview with estimated size, fees, slippage, minimum trade rules, exposure, and final-order checks, then shows a green `SAFE READY FOR OPTIONAL TINY LIVE TEST` card when Kraken auth, no-order preview, permission safety, and endpoint-lock proof pass. Execution remains blocked even after typed owner confirmation.
- Phase 6F still cannot place a real order. It keeps Kraken AddOrder, unrestricted live trading, autonomous loops, scaling, withdrawals/transfers, wallet signing, margin, futures, and leverage disabled.
- The next owner-gated step is a Kraken-only `$1 Tiny Live Test` flow on `/live-trading-launch`. It adds `GET/POST /api/v1/live-trading-launch/kraken-tiny-live-test/...` routes for status, exact preview, final placement, order status, cancel, and emergency stop. The final `Place Tiny Live Test Order` button stays disabled until the no-order preview is ready, the owner types `I APPROVE THIS $1 KRAKEN TINY LIVE TEST`, checks manual approval, and arms Emergency Stop. The backend then re-checks Kraken auth, unsafe permissions, dry-run proof, balance, minimum order size, risk profile, kill switch, duplicate prevention, and one-order-only rules before any Kraken AddOrder call.
- The Kraken tiny-live flow does not enable autonomous loops, retries, scaling, withdrawals/transfers, wallet signing, margin, futures, or leverage. If Kraken balance/minimum-size rules block the $1 test, Simple Mode explains the exact blocker and leaves the production endpoint uncalled.
- Kraken-specific sandbox evidence is no longer shown as a Simple Mode blocker once authenticated Kraken dry-run/preview proof has passed. Balance and minimum-size issues are preserved as final order gates, not setup failures.
- The normal Simple Mode path now adds one giant `Start Kraken Live Setup Walkthrough` button above the Phase 6E/6F technical panels. It walks the owner through creating the restricted Kraken spot key, confirming withdrawals/transfers/margin/futures/leverage are off, saving the key into the encrypted vault, verifying the connection, running dry-run proof, and checking tiny live eligibility with plain-English instructions and visible fix lists.
- Kraken authentication diagnostics are now available inside the same owner walkthrough. `GET /api/v1/live-trading-launch/kraken-auth-diagnostics/status` checks encrypted-vault presence and local nonce/signature readiness without calling Kraken. `POST /api/v1/live-trading-launch/kraken-auth-diagnostics/raw-balance` tests only Kraken `/0/private/Balance`, records the exact HTTP response code/body, classifies invalid key/signature/permission/nonce/clock/vault/encoding issues, shows redacted fingerprints only, and keeps live trading/order endpoints disabled.
